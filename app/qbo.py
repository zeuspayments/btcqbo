...
def post_payment(doc_number="", amount=0, btcp_id=''):
    ...
    qb = fetch('qbclient')
    # check if ZEUSPay is already in QBO as a pmt method
    pmt_method_list = PaymentMethod.filter(
        Name="ZEUSPay",
        qb=qb
    )
    try:
        # if ZEUSPay is already in QBO, set it as pmt method
        pmt_method = pmt_method_list[0]
    except IndexError:
        # if ZEUSPay is not in QBO, create it as pmt method
        new_pmt_method = PaymentMethod()
        new_pmt_method.Name = "ZEUSPay"
        new_pmt_method.save(qb=qb)
        # set newly created ZEUSPay pmt method as pmt method
        pmt_method_list = PaymentMethod.filter(
            Name="ZEUSPay",
            qb=qb
        )
        pmt_method = pmt_method_list[0]
    # check if QBO has asset acct for Bitcoin-ZEUSPay
    deposit_acct_list = Account.filter(
        Name="Bitcoin-ZEUSPay",
        qb=qb
    )
    try:
        # if Bitcoin-ZEUSPay is in QBO, set as deposit acct
        deposit_acct = deposit_acct_list[0]
    except IndexError:
        # if Bitcoin-ZEUSPay is not in QBO, create it as deposit acct
        new_acct = Account()
        new_acct.Name = "Bitcoin-ZEUSPay"
        new_acct.AccountSubType = "OtherCurrentAssets"
        new_acct.save(qb=qb)
        # set newly created Bitcoin-ZEUSPay acct as deposit acct
        deposit_acct_list = Account.filter(
            Name="Bitcoin-ZEUSPay",
            qb=qb
        )
...
def post_deposit(amount, tax, btcp_id):
    ...
    refresh_stored_tokens()
    qb = fetch('qbclient')
    # check if ZEUSPay income  acct is already in QBO
    ...
    try:
        ...
        income_acct = income_acct_list[0]
    # check if ZEUSPay Sales Tax acct is already in QBO
    sales_tax_acct_list = Account.filter(
        Name="Sales Tax from ZEUSPay",
        qb=qb
    )
    try:
        ...
        new_acct = Account()
        new_acct.Name = "Sales Tax from ZEUSPay"
